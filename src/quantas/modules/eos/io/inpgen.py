...
from quantas.core.geometry.symmetry import analyze_symmetry
...
EnergyVolumeParseResult = CrystalEnergyVolumeParseResult | VaspEnergyVolumeParseResult


@dataclass(frozen=True, slots=True)
class _CrystalReferenceInfo:
    """Reference-cell transformation and symmetry for generated EOS input."""
...
class EOSEnergyInputCreator:
    ...
    interface : str, optional
        Backend interface identifier. ``"crystal"`` and ``"vasp"`` are supported.
    ...
    def __init__(
        ...
        self.crystal_reference = parse_crystal_reference(crystal_reference)
        self.files: list[Path] = []
        self.results: list[EnergyVolumeParseResult] = []
        self.series: StructureEnergySeries | None = None
        self.crystal_info: _CrystalReferenceInfo | None = None
    ...
    def read(
        ...
        *,
        ...
        self.emit(
            f"Reading {len(files)} Energy EOS source(s) with the {self.interface} interface",
            data={
        ...
        results: list[EnergyVolumeParseResult] = []
        ...
        self._validate_energy_signatures(results)
        self._validate_source_normalizations(results)
        ...
        series = StructureEnergySeries(
            ...
            metadata={
                ...
                "energy_signature": tuple(results[0].energy_signature),
                **(
                    {"corrections": results[0].corrections}
                    if isinstance(results[0], CrystalEnergyVolumeParseResult)
                    else {}
                ),
            },
        )
        ...
        self.series = series
        self.crystal_info = self._crystal_reference_info(series, results)
        self.emit(
    ...
    def write(
        ...
        *,
        ...
        if self.crystal_info is None:
            raise RuntimeError("Energy EOS crystal-reference metadata is unavailable")
        path = Path(destination)
        rows = [
            _format_point(
                point,
                reference=self.crystal_reference,
                info=self.crystal_info,
            )
        ...
        comments = [
            ...
            for index, source in enumerate(self.series.metadata.get("sources", ()))
        ]
        comments.append(
            "COMMENT V, E, and lattice parameters use the declared CRYSTAL_REFERENCE; "
            "CELL_MULTIPLICITY is the crystallographic/primitive cell-volume ratio."
        )
        symmetry = self.crystal_info.symmetry
        system = crystal_system_from_space_group_number(symmetry.space_group_number)
        text = "\n".join(
            [
                f"JOB {jobname}",
                ...
                *comments,
                ...
                f"SPACE_GROUP_SYMBOL {symmetry.international_symbol}",
                f"CRYSTAL_REFERENCE {self.crystal_reference.value}",
                f"CELL_MULTIPLICITY {self.crystal_info.multiplicity}",
                (
    ...
    @staticmethod
    def _crystal_reference_info(
        series: StructureEnergySeries,
        results: Sequence[EnergyVolumeParseResult],
    ) -> _CrystalReferenceInfo:
        """Return one constant reference-cell transformation for the series."""
        reference = series.points[series.reference_index].structure
        symmetry = analyze_symmetry(reference, symprec=1.0e-5)
        explicit_numbers = {
            int(result.space_group_number)
            for result in results
            if result.space_group_number is not None
        }
        if explicit_numbers and explicit_numbers != {symmetry.space_group_number}:
            rendered = ", ".join(str(value) for value in sorted(explicit_numbers))
            raise ValueError(
                "source interface and spglib disagree on the reference space group: "
                f"output={rendered}, spglib={symmetry.space_group_number}"
            )

        transforms = [
            np.asarray(result.primitive_to_crystallographic, dtype=np.float64)
            for result in results
            if result.primitive_to_crystallographic is not None
        ]
        if transforms:
            first = transforms[0]
            if any(
                not np.allclose(item, first, rtol=0.0, atol=1.0e-10)
                for item in transforms[1:]
            ):
                raise ValueError(
                    "Energy EOS source files use inconsistent "
                    "primitive-to-crystallographic transformations"
                )
            basis = first.T
            source = "crystal_output"
        else:
            transformation = symmetry.transformation_matrix
            if transformation is None:
                ...
                if abs(determinant) <= np.finfo(np.float64).eps:
                    raise ValueError("spglib returned a singular standard transformation")
                basis = np.asarray(np.linalg.inv(transformation).T, dtype=np.float64)
                source = "spglib_standard"
        factor = abs(float(np.linalg.det(basis)))
        multiplicity = int(round(factor))
        if multiplicity < 1 or not np.isclose(
            ...
            raise ValueError(
                "primitive-to-crystallographic transformation has a non-integral "
                f"volume factor: {factor:.12g}"
            )
        return _CrystalReferenceInfo(
            symmetry=symmetry,
            basis_matrix=basis,
        ...
        *,
        ...
        for path in files:
            ...
            if not normalized.exists():
                raise ValueError(f"Energy EOS source does not exist: {path}")
            seen.add(normalized)
    ...
    @staticmethod
    def _validate_energy_signatures(
        results: Sequence[EnergyVolumeParseResult],
    ) -> None:
        """Reject sources that describe different electronic-energy surfaces."""
        signatures = {tuple(result.energy_signature) for result in results}
        if len(signatures) == 1:
            return
        rendered = " | ".join(
        ...
        if all(
            isinstance(result, CrystalEnergyVolumeParseResult) for result in results
        ):
            raise ValueError(
        ...
        raise ValueError(
            "Energy EOS sources use incompatible energy semantics/settings: "
            f"{rendered}"
        )
    ...
    def _validate_source_normalizations(
        ...
        """Require one stable source-to-primitive basis across VASP runs."""
        matrices = [
            np.asarray(result.source_to_primitive, dtype=np.float64)
            for result in results
            if isinstance(result, VaspEnergyVolumeParseResult)
        ]
        if len(matrices) <= 1:
            return
        reference = matrices[0]
        if any(
            not np.allclose(matrix, reference, rtol=0.0, atol=1.0e-8)
            for matrix in matrices[1:]
        ):
            raise ValueError(
                "VASP Energy EOS sources do not share one stable "
                "source-to-primitive cell transformation"
            )
...
def create_eos_energy_input(
    ...
    *,
    ...
    crystal_reference : CrystalReference or str, optional
        Cell reference written to the generated dataset. ``primitive`` keeps
        the electronic-structure normalization; ``crystallographic`` applies
        one constant reference transformation and scales energy and volume by
        the associated cell multiplicity.
...
def _format_point(
    ...
    *,
    ...
    info: _CrystalReferenceInfo,
) -> str:
    """Serialize one Energy EOS state without display-precision rounding."""
    if reference is CrystalReference.CRYSTALLOGRAPHIC:
        lattice = info.basis_matrix @ np.asarray(point.structure.lattice, dtype=np.float64)
        parameters = lattice_parameters(lattice)
        volume = _point_volume(point) * info.multiplicity
        energy = float(point.energy.value) * info.multiplicity
    else:
        parameters = _cell_parameters(point)
        volume = _point_volume(point)
        energy = float(point.energy.value)
    values = [
        volume,
        *parameters.tolist(),
        energy,

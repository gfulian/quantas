...
Generated Energy EOS tables
---------------------------

The Energy EOS input generator writes the complete cell metrics associated with
each electronic-structure energy::

   JOB MgO static energy-volume data
   PROVENANCE Generated by Quantas from CRYSTAL structure-energy output
   SYSTEM cubic
   SPACE_GROUP_NUMBER 225
   SPACE_GROUP_SYMBOL Fm-3m
   CRYSTAL_REFERENCE primitive
   CELL_MULTIPLICITY 4
   UNITS V=angstrom^3 A=angstrom B=angstrom C=angstrom ALPHA=degree BETA=degree GAMMA=degree E=Ha
   FORMAT V A B C ALPHA BETA GAMMA E
   DATA
   17.111474 2.892295 2.892295 2.892295 60 60 60 -275.1702459
   18.803818 2.984835 2.984835 2.984835 60 60 60 -275.1739316

The generated file records data only.  The E(V) formulation, fit constraints,
and numerical method belong to the EOS request or specification file.  Quantas
...
as synthetic observations by ``inpgen``.  When a complete lattice path is
present, the same fit also derives equilibrium cell lengths and axial moduli
through the shared structural-path model.

``quantas eos inpgen`` analyses the symmetry of one reference structure and
keeps that crystallographic interpretation fixed over the complete volume
series.  By default ``CRYSTAL_REFERENCE primitive`` preserves the normalization
of the electronic-structure calculation.  ``--crystal-reference
crystallographic`` instead applies one constant primitive-to-crystallographic
transformation and scales *both* energy and volume by ``CELL_MULTIPLICITY``.
The pressure relation is therefore unchanged.  For CRYSTAL, an explicit
primitive-to-crystallographic transformation printed by the code has priority;
...
``SPACE_GROUP_NUMBER``
   Optional international space-group number (1--230). If ``SYSTEM`` is also
   provided, both declarations must identify the same crystal system.

``SPACE_GROUP_SYMBOL``
   Optional Hermann--Mauguin symbol retained as structural provenance.

``CRYSTAL_REFERENCE``
   Optional cell-reference declaration: ``primitive`` or ``crystallographic``.
   The alias ``conventional`` is accepted by the Python API and normalized to
   ``crystallographic``.  In generated Energy EOS files, volume, energy, and
   all six lattice parameters use the same declared reference.

``CELL_MULTIPLICITY``
   Optional positive integer giving the crystallographic/primitive cell-volume
   ratio.  For example, the conventional cubic cell of an FCC lattice has
   multiplicity four relative to its primitive cell.
...
``UNITS``
   ...
   ``UNITS P=GPa V=angstrom^3 E=Ha``. Absolute pressure, length/volume,
   temperature, energy, and matching uncertainty columns are converted to the
   canonical in-memory EOS units. Raw values and source-unit labels are retained
   for provenance. A normalized ``VSCALE`` or ``LSCALE`` is incompatible with a
   dimensional unit declaration and is rejected rather than silently
   reinterpreted.
...
self-contained HDF5 persistence.
...
:command:`quantas eos run` accepts ``--energy-unit``, ``--pressure-unit``,
``--length-unit``, and ``--temperature-unit``. Explicit command-line units
override declarations in the input file. When ``--spec`` is used, the
corresponding ``[input]`` entries are authoritative and conflicting scientific
CLI overrides are rejected. Without an explicit source, EOS assumes Hartree,
